...
import { ArrowLeft, CalendarRange, Clock3, Filter, Loader2, Sparkles, Trophy, ChevronRight } from "lucide-react"
...
export default function TournamentFixtureClient({ tournamentId }: TournamentFixtureClientProps) {
  ...
  useEffect(() => {
    const channel = supabase
      .channel(`tournament-fixture-${tournamentId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  const roundOptions = useMemo(() => {
    ...
    const values = Array.from(new Set(filteredByGroup.map((match) => match.round_number).filter((n): n is number => n !== null)))
          ...
          <div className="flex items-center gap-2">
            ...
            {hasFixture && (
              <>
                ...
                {isOwner && tournament?.has_knockout && (
                  <Link
                    href={`/tournaments/${tournamentId}/knockout`}
