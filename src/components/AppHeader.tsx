...
export function AppHeader() {
  ...
  const handleMobileClose = () => setMobileExpanded(false);

  // Sync sidebar width as CSS variable for main content margin
  useEffect(() => {
    const update = () => {
      if (!isLoggedIn) {
        document.documentElement.style.setProperty("--sidebar-width", "0px");
      } else if (window.innerWidth < 768) {
        // Mobile: sidebar always collapsed (64px), expanded overlays content
        document.documentElement.style.setProperty("--sidebar-width", "64px");
      } else {
        document.documentElement.style.setProperty("--sidebar-width", sidebarExpanded ? "208px" : "64px");
      }
    };
    update();
    window.addEventListener("resize", update);
    return () => window.removeEventListener("resize", update);
  }, [isLoggedIn, sidebarExpanded]);
      ...
      <header className="sticky top-0 z-50 bg-card/80 backdrop-blur-md border-b border-border/60 shadow-sm py-3 px-4 sm:py-4 sm:px-6">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            ...
            {/* Desktop nav — only when NOT logged in */}
            {!isLoggedIn && (
