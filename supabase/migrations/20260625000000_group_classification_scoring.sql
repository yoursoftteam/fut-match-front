...
CREATE INDEX idx_bet_best_third_pool ON bet_best_third_qualifiers(pool_id);

-- 2. Function: calculate simulated group standings for a user
...
BEGIN
  ...
  WITH group_teams AS (
    ...
    WHERE bm.tournament_id = p_tournament_id
      AND bm.group_name = p_group_name
  ),
  ...
  team_stats AS (
    SELECT
      gt.id,
      gt.name,
      COUNT(*) FILTER (WHERE mp.home_team_id = gt.id OR mp.away_team_id = gt.id) AS played,
      COUNT(*) FILTER (
        WHERE mp.home_team_id = gt.id AND mp.home_goals > mp.away_goals
          OR mp.away_team_id = gt.id AND mp.away_goals > mp.home_goals
      ) AS wins,
      COUNT(*) FILTER (
        WHERE mp.home_team_id = gt.id AND mp.home_goals = mp.away_goals
          OR mp.away_team_id = gt.id AND mp.away_goals = mp.home_goals
      ...
      SUM(CASE WHEN mp.home_team_id = gt.id THEN mp.home_goals ELSE mp.away_goals END) AS goals_for,
      SUM(CASE WHEN mp.home_team_id = gt.id THEN mp.away_goals ELSE mp.home_goals END) AS goals_against
    FROM group_teams gt
  ...
  SELECT
    ...
    (ts.wins * 3 + ts.draws)::INT AS points
  FROM team_stats ts
  ORDER BY points DESC, (goals_for - goals_against) DESC, goals_for DESC;
END;
$$;

-- 3. Function: calculate actual group standings from official results
...
BEGIN
  ...
  match_results AS (
    SELECT
      bm.home_team_id,
      bm.away_team_id,
      COALESCE(bm.home_score_official, 0) AS home_goals,
      COALESCE(bm.away_score_official, 0) AS away_goals
    FROM bet_matches bm
    WHERE bm.tournament_id = p_tournament_id
      AND bm.group_name = p_group_name
  ...
  team_stats AS (
    SELECT
      gt.id,
      gt.name,
      COUNT(*) FILTER (WHERE mr.home_team_id = gt.id OR mr.away_team_id = gt.id) AS played,
      COUNT(*) FILTER (
        WHERE mr.home_team_id = gt.id AND mr.home_goals > mr.away_goals
          OR mr.away_team_id = gt.id AND mr.away_goals > mr.home_goals
      ) AS wins,
      COUNT(*) FILTER (
        WHERE mr.home_team_id = gt.id AND mr.home_goals = mr.away_goals
          OR mr.away_team_id = gt.id AND mr.away_goals = mr.home_goals
      ...
      SUM(CASE WHEN mr.home_team_id = gt.id THEN mr.home_goals ELSE mr.away_goals END) AS goals_for,
      SUM(CASE WHEN mr.home_team_id = gt.id THEN mr.away_goals ELSE mr.home_goals END) AS goals_against
    FROM group_teams gt
...
-- 4. Main function: calculate group classification points for a pool
...
BEGIN
  ...
  SELECT pts_qualified_round_2 INTO v_pts_qualified
  FROM bet_pool_config_versions WHERE id = v_config_id;

  IF v_pts_qualified IS NULL OR v_pts_qualified <= 0 THEN
    RETURN QUERY SELECT 0, 0, false;
    RETURN;
  END IF;
  ...
  FROM bet_matches bm
  WHERE bm.tournament_id = v_tournament_id
    AND bm.group_name = p_group_name;

  IF NOT v_all_finished THEN
    RETURN QUERY SELECT 0, 0, false;
    RETURN;
  END IF;
  ...
  -- For each pool member with predictions for this group, check simulated top 2
  FOR v_user IN
    SELECT DISTINCT bmp.user_id
    FROM bet_match_predictions bmp
    JOIN bet_matches bm ON bm.id = bmp.match_id
    WHERE bmp.pool_id = p_pool_id
      AND bmp.mode = 'pool'
      AND bm.tournament_id = v_tournament_id
      AND bm.group_name = p_group_name
  LOOP
    -- Get simulated top 2 for this user
    ...
    -- Compare top 2 regardless of order
    IF v_sim_top2 IS NOT NULL AND array_length(v_sim_top2, 1) >= 2 THEN
      IF (v_sim_top2[1] = v_actual_top2[1] AND v_sim_top2[2] = v_actual_top2[2])
         OR (v_sim_top2[1] = v_actual_top2[2] AND v_sim_top2[2] = v_actual_top2[1]) THEN
        -- Remove old classification details for this user/group
        DELETE FROM bet_scores_details
        WHERE source_type = 'classification'
          AND source_id = 'group_' || p_group_name
          AND mode = 'pool'
          AND pool_id = p_pool_id
          AND user_id = v_user.user_id;

        -- Insert new classification points
  ...
  -- Rebuild aggregate for affected users
  IF v_affected > 0 THEN
    WITH affected_users AS (
      SELECT DISTINCT user_id FROM bet_scores_details
      WHERE mode = 'pool' AND pool_id = p_pool_id
    ),
    user_totals AS (
...
BEGIN
  ...
  FOR v_third_qualifiers IN
    SELECT bbtq.group_name, bbtq.team_id
    FROM bet_best_third_qualifiers bbtq
    WHERE bbtq.pool_id = p_pool_id
  LOOP
    FOR v_user IN
      SELECT DISTINCT bmp.user_id
      FROM bet_match_predictions bmp
      ...
      WHERE bmp.pool_id = p_pool_id
        AND bmp.mode = 'pool'
        AND bm.group_name = v_third_qualifiers.group_name
    LOOP
      -- Get the 3rd place team in user's simulated standings
      SELECT team_id INTO v_sim_third
      FROM (
        SELECT team_id, ROW_NUMBER() OVER (ORDER BY points DESC, (goals_for - goals_against) DESC, goals_for DESC) AS rank
        FROM fn_user_simulated_group_standings(
          v_user.user_id, p_pool_id, v_third_qualifiers.group_name,
          (SELECT tournament_id FROM bet_pools WHERE id = p_pool_id)
        )
      ) sub
      WHERE rank = 3;

      IF v_sim_third = v_third_qualifiers.team_id THEN
        DELETE FROM bet_scores_details
        WHERE source_type = 'classification'
          AND source_id = 'best_third_' || v_third_qualifiers.group_name
          AND mode = 'pool'
          AND pool_id = p_pool_id
          AND user_id = v_user.user_id;

        INSERT INTO bet_scores_details (mode, pool_id, user_id, source_type, source_id, points)
        VALUES ('pool', p_pool_id, v_user.user_id, 'classification', 'best_third_' || v_third_qualifiers.group_name, v_pts_qualified);
...
-- 6. Update fn_update_match_result to trigger group classification when group finishes
...
DECLARE
  ...
  v_pool_id UUID;
  v_match_group CHAR(1);
  v_tournament_id UUID;
  v_all_group_finished BOOLEAN;
BEGIN
  ...
  -- If this was a group stage match, check if the whole group is now finished
  -- and if so, calculate classification points for each pool
  IF v_stage = 'group_stage' AND v_match_group IS NOT NULL THEN
    SELECT bool_and(bm.status = 'finished') INTO v_all_group_finished
    FROM bet_matches bm
    WHERE bm.tournament_id = v_tournament_id
      AND bm.group_name = v_match_group;
